...
void Xu_Li_Con_Tro_Chi_Vi(int CursorVisibility)
{
	HANDLE handle = GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleCursorInfo(handle, new CONSOLE_CURSOR_INFO({ 20, CursorVisibility }));
}
...
void VeThem_GV_SV(int SLCH) {
	int line = SLCH - 3;
	int hang = line * 6 - SLCH;
	gotoXY(42, 28);
	cout << "                                                                                         ";
